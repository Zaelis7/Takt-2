...
| Arbeitspakete | 108 | 38 implemented, 68 planned, 0 in progress, 2 durch dokumentierte Entscheidungen blockiert |
| Offene Findings | 5 | 4 Spec-/Contract-/Implementierungs-/Owner-Themen und 1 Evidence-Lücke; alle fünf high |
...
| Öffentliche Systemgrenze | Health und komponierte Login-/Session-/Logout-Routen mit sicheren Cookies, CSRF und festem 10/Minute-IP-/Kontolimit; API-Token List/Get liefern produktiv komponiert redigierte Metadaten, typisierte Filter, signierte Cursorseiten, ETag, generische Credentials und typisierte Problems | API-Token-Writes, Recovery-Runtime, weitere `/api/v1`-Ressourcen, allgemeine AuthZ, Metrics und Traces fehlen |
...
| `SPEC-008` | Patch/Revoke speichern nur Token-ID und Ergebnisversion; nach einer späteren Mutation kann die laut Architektur erforderliche ursprüngliche 24-Stunden-Antwort noch nicht rekonstruiert werden. |
...
1. `IAM-043`, `IAM-040`, `IAM-041`, `IAM-013`: zuerst wird der in `SPEC-008` fehlende sichere Patch-/Revoke-Antwortsnapshot ergänzt; danach folgen Create- und Patch/Revoke-HTTP sowie abschließend die Produktionskomposition.
