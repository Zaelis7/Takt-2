...
| Arbeitspakete | 102 | 34 implemented, 66 planned, 0 in progress, 2 durch dokumentierte Entscheidungen blockiert |
...
| Öffentliche Systemgrenze | Health und komponierte Login-/Session-/Logout-Routen mit sicheren Cookies, CSRF und festem 10/Minute-IP-/Kontolimit; API-Token-CRUD ist vertraglich definiert, die injizierbaren List-/Get-Grenzen liefern redigierte Metadaten, typisierte Filter, signierte Cursorseiten, ETag, generische Credentials und typisierte Problems | Produktionskomposition für API-Token-Reads, Token-Writes, Recovery-Runtime, weitere `/api/v1`-Ressourcen, allgemeine AuthZ, Metrics und Traces fehlen |
...
1. `IAM-034`, `IAM-013`: die Produktionskomposition der API-Token-Read-Grenzen und anschließend die Write-Runtime auf den vorhandenen Verwaltungs-, Bearer-, Idempotenz- und Cursorgrundlagen umsetzen.
