...
| Arbeitspakete | 108 | 40 implemented, 66 planned, 0 in progress, 2 durch dokumentierte Entscheidungen blockiert |
| Offene Findings | 4 | 2 Spec-/Contract-Themen, 1 Owner-Entscheidung und 1 Evidence-Lücke; alle vier high |
...
| Öffentliche Systemgrenze | Health und komponierte Login-/Session-/Logout-Routen mit sicheren Cookies, CSRF und festem 10/Minute-IP-/Kontolimit; API-Token List/Get liefern produktiv komponiert redigierte Metadaten, typisierte Filter, signierte Cursorseiten, ETag, generische Credentials und typisierte Problems; eine isolierte Create-Grenze ergänzt bedingtes Browser-CSRF, kanonische Idempotenz und die einmalige Tokenantwort | Produktionskomposition der API-Token-Writes, Patch/Revoke-HTTP, Recovery-Runtime, weitere `/api/v1`-Ressourcen, allgemeine AuthZ, Metrics und Traces fehlen |
...
| Domain/Application | Browser-Login, Session-Aktivität/CSRF-Rotation und Logout orchestrieren Credential-Prüfung, 256-Bit-Werte, Digestgrenzen, Kontext und Audit frameworkfrei und sind mit HTTP/Persistenz komponiert; API-Token besitzen redigierte Secret-/Argon2id-Grenzen, kanonische CIDRs, exakte Scopes, Actor/Methode/Pfad/Key/Hash-gebundene Idempotenz, permission- und kontextgeprüfte Verwaltung, fail-closed Bearer-Authentifizierung mit monotonem Last-used, komponierte Read-Use-Cases sowie actor-gebundene idempotente Create-/Patch-/Revoke-Orchestrierung mit sicherem verzögertem Mutations-Replay | API-Token-Write-HTTP-Komposition, Recovery-HTTP-Orchestrierung, Monitore, Scheduler, Evaluator, Uptime, Outbox und allgemeine Permission Engine fehlen |
| Persistenz | PostgreSQL-/SQLite-Migrationen `0001` bis `0007`, Identitäts-, Session-, Recovery- und API-Token-Repositories sowie atomare Create-/Patch-/Revoke-Idempotenz mit Konkurrenz-, Konflikt-, Ablauf- und Rollbackparität; Create-Replay bleibt innerhalb von 24 Stunden auch nach Token-Ablauf verfügbar, Patch/Revoke bewahren Status, ETag und sichere Originalprojektion trotz späterer Mutation, Token-Lifecycle ist optimistisch versioniert und Schreibakteure bleiben wahrheitsgetreu | Secret Store, Monitor/Revision, Job/Observation/Evaluation, Outbox, Statusseite und Retention fehlen |
...
`SPEC-001` ist gelöst: Die Daten-IDs sind kanonisch nachverfolgt. `SPEC-013` vereinheitlicht die prüfungsspezifischen CheckSpec-Felder; `SPEC-019` ergänzt die gemeinsamen Proxy-, Resolver- und Adressfamilienoptionen mit denselben Namen, Grenzen und SecretRefs in allen drei Maschinenverträgen. Damit ist `SPEC-004` vollständig gelöst. `SPEC-005` ist durch Entfernen des historisch unbelegten Template-Eintrags und einen CI-geprüften Spec-Index gelöst. `SPEC-006` ist durch den eindeutigen, verschlüsselten und auf 24 Stunden begrenzten API-Token-Create-Replay-Vertrag gelöst. `SPEC-007` ist durch den OpenAPI-gebundenen `invalid_cursor`-Problemvertrag gelöst. `SPEC-008` ist durch den maschinenlesbaren Patch/Revoke-Replayvertrag, Migration `0007` und engine-paritäre sichere Antwortsnapshots gelöst. `SEC-001` und `SEC-002` sind durch gepinnte sichere `js-yaml`-/`fast-uri`-Auflösungen, Lockfile-Regressionsfälle und den grünen vollständigen Node-Audit gelöst. `SEC-003` ist durch typisierte API-Token-Akteure, Migration `0006` und gemeinsame PostgreSQL-/SQLite-Verträge gelöst. `EVID-001` ist durch den unabhängigen Clean-Checkout-Verdict für Commit `4ef411a` gelöst, ohne den historischen fehlgeschlagenen Bootstrap-Verdict zu überschreiben.
...
1. `IAM-041`, `IAM-013`: Patch/Revoke-HTTP nutzt den jetzt vollständigen sicheren Replayvertrag; abschließend folgt die Produktionskomposition einschließlich Create.
...
## Abschluss von IAM-035

`IAM-035` ist `implemented`, nicht `verified`. Die echte Axum-Listengrenze übernimmt die redigierende Credential-Grenze aus `IAM-037`, validiert `limit`, `project_id`, `kind`, `status` und `scope` strikt und genau einmal, dekodiert den IAM-036-Cursor erst nach vollständiger Filterkanonisierung und gibt ausschließlich sichere Metadaten in stabiler Reihenfolge `created_at desc, id desc` aus. Manipulierte oder filterfremde Cursor sowie ungültige, doppelte oder unbekannte Querywerte scheitern vor Application mit dem einzigen für die Operation vertraglich definierten `400 invalid_cursor`; Port-Antworten mit Filter-, Sortier-, Seitengrenzen- oder Projektionsdrift scheitern geschlossen als interner Fehler. 448 Source-/Test-Einfügungen sowie drei Manifestzeilen bleiben unter der 550-Zeilen-Schätzung. Sämtliche lokalen Repository-Gates einschließlich PostgreSQL 16.9, SQLite, Supply Chain, Frontend, Playwright und Release-Build sind grün; die 37 Produkt-Acceptance-Bindings bleiben geplant. Produktionskomposition folgt in `IAM-034`; unabhängige commit-gebundene Review-/CI-Evidence fehlt. Details stehen in `docs/implementation-evidence/iam-035-api-token-read-boundary.md`.

## Abschluss von IAM-034

`IAM-034` ist `implemented`, nicht `verified`. Die Produktionskomposition verbindet API-Token List/Get mit read-only Browser-Session-Authentifizierung oder exakt einem Bearer-Credential, prüft `api_tokens:read` sowie Organisations-/Projektkontext vor der Repository-Abfrage und gibt ausschließlich redigierte Metadaten aus. Der echte SQLite-HTTP-Test deckt gefilterte Listen, Get/ETag, fremden Projektkontext, falschen Scope, ungültige Credentials und Secret-Negativfälle ab; ein unverändertes CSRF-Token belegt, dass Reads keine Sessionrotation auslösen. PostgreSQL 16.9 und SQLite bestehen weiterhin denselben Repository-Vertrag. 792 Source-/Test-Einfügungen sowie zwei Manifestzeilen bleiben unter dem 800-Zeilen-Limit; alle lokalen Gates sind grün. API-Token-Writes folgen in `IAM-013`; unabhängige commit-gebundene Review-/CI-Evidence fehlt. Details stehen in `docs/implementation-evidence/iam-034-api-token-read-http.md`.

## Abschluss von IAM-038

`IAM-038` ist `implemented`, nicht `verified`. Der Preflight von `IAM-013` zeigte, dass laut OpenAPI zulässige Bearer-Schreibzugriffe im bisherigen Audit-/Idempotenzschema nur fälschlich einem Benutzer oder System hätten zugerechnet werden können; `SEC-003` wurde deshalb vor der HTTP-Umsetzung geschlossen. Domain und Persistenz unterscheiden nun Benutzer- und API-Token-Akteure typisiert. Forward-only Migration `0006` ergänzt auf PostgreSQL und SQLite gegenseitig ausgeschlossene Akteurspalten, einen restriktiven Token-Fremdschlüssel und den Idempotenz-Akteurwert `api_token`. Derselbe Engine-Vertrag belegt actor-gebundenes Create/Replay, exakten Audit-Readback, Fremdschlüssel-/Mischidentitätsablehnung und Secret-Redaktion. 684 handgeschriebene Source-/Test-/Migrations-/Dokumentationszeilen bleiben unter dem 800-Zeilen-Limit; alle lokalen Gates einschließlich PostgreSQL 16.9, SQLite, Supply Chain, Frontend, Playwright und Release-Build sind grün. Write-Orchestrierung und HTTP folgen in `IAM-039` bis `IAM-041` und `IAM-013`; unabhängige commit-gebundene Review-/CI-Evidence fehlt. Details stehen in `docs/implementation-evidence/iam-038-api-token-actor.md`.

## Abschluss von IAM-039

`IAM-039` ist nach der verpflichtenden Größenaufteilung `implemented`, nicht `verified`. Der kombinierte Create-/Patch-/Revoke-Entwurf maß 876 handgeschriebene Einfügungen und wurde deshalb in diese Create-Orchestrierung und das neue Folgepaket `IAM-042` für Patch/Revoke geteilt. Browser und Bearer mit exaktem `api_tokens:write` werden vor Fachwirkung an Organisation und Projekt gebunden; Create persistiert Token, wahrheitsgetreues Audit und verschlüsselten Replay atomar. Identischer Replay liefert denselben einmaligen Tokenwert und dieselbe sichere Erstellungsprojektion ohne weitere Fach- oder Auditwirkung, auch wenn der Token innerhalb des 24-Stunden-Idempotenzfensters bereits abgelaufen ist. 731 Source-/Test-Einfügungen bleiben unter dem 800-Zeilen-Limit. Sämtliche lokalen Repository-Gates einschließlich PostgreSQL 16.9, SQLite, Supply Chain, Frontend, Playwright und Release-Build sind grün; der v0.1-Readiness-Runner bleibt wegen 15 geplanter Szenarien ehrlich rot. Patch/Revoke-Application folgt in `IAM-042`, Write-HTTP in `IAM-040`/`IAM-041` und die Produktionskomposition in `IAM-013`; unabhängige commit-gebundene Review-/CI-Evidence fehlt. Details stehen in `docs/implementation-evidence/iam-039-api-token-idempotent-writes.md`.

## Abschluss von IAM-042

`IAM-042` ist `implemented`, nicht `verified`. Die frameworkfreie API-Token-Write-Grenze orchestriert jetzt auch Patch und Revoke für permission- und kontextgeprüfte Browser- sowie exakt `api_tokens:write`-berechtigte Bearer-Akteure. Actor, Methode, Pfad, Key und Request-Hash werden vor dem atomaren Repository-Write gebunden; identische unmittelbare Replays wiederholen weder Mutation noch Audit, Hash-/Versionskonflikte bleiben wirkungslos und sämtliche Mutationsausgaben sind secret-frei. 516 Source-/Test-Einfügungen bleiben unter der 550-Zeilen-Schätzung. Alle lokalen Repository-Gates einschließlich PostgreSQL 16.9, SQLite, Supply Chain, Frontend, Playwright und Release-Build sind grün; der v0.1-Readiness-Runner bleibt wegen 15 geplanter Szenarien ehrlich rot. Der Review fand `SPEC-008`: Nach einer weiteren Tokenmutation reicht die bisher persistierte ID-/Versionsreferenz nicht für die laut Architektur identische ursprüngliche Patch-Antwort. `IAM-043` behebt das vor der HTTP-Freigabe; bis dahin scheitert die Application geschlossen. Unabhängige commit-gebundene Review-/CI-Evidence fehlt. Details stehen in `docs/implementation-evidence/iam-042-api-token-idempotent-mutations.md`.

## Abschluss von IAM-043

`IAM-043` ist `implemented`, nicht `verified`. OpenAPI beschreibt Patch-/Revoke-Replays nun actor-/method-/path-/hash-gebunden und für exakt 24 Stunden mit ursprünglichem Status, relevantem ETag und sicherem Body. Forward-only Migration `0007` speichert auf PostgreSQL und SQLite eine streng gelesene redigierte Tokenprojektion ohne Tokenwert, Hash oder Create-Ciphertext. Application-Replays verwenden ausschließlich diesen Snapshot, behalten dadurch auch Status und Version nach einer späteren Mutation und erzeugen weder einen zweiten Write noch ein Audit-Event. 689 handgeschriebene Contract-/Source-/Test-/Migrations-Einfügungen bleiben unter der 700-Zeilen-Schätzung und dem 800-Zeilen-Limit. Alle lokalen Repository-Gates einschließlich echtem PostgreSQL 16.9, SQLite, Supply Chain, Frontend, Playwright und Release-Build sind grün; der v0.1-Readiness-Runner bleibt wegen 15 geplanter Szenarien ehrlich rot. Unabhängige commit-gebundene Review-/CI-Evidence fehlt. Details stehen in `docs/implementation-evidence/iam-043-api-token-mutation-replay.md`.

## Abschluss von IAM-040

`IAM-040` ist `implemented`, nicht `verified`. Die echte Axum-Create-Grenze validiert DTO, Credential und optionale Idempotenz strikt, fordert CSRF ausschließlich für Browser-Sessions und bindet den kanonischen Request-Hash an genau einen typisierten Port. Create und identischer Replay liefern den vertraglichen Status `201`, `Location`, `ETag` und denselben einmaligen Tokenwert; alle Fehler und fail-closed Projektionstests bleiben secret-frei. Der erste vollständige Entwurf lag mit 820 Einfügungen über dem Limit und wurde ohne Verlust der Acceptance-/Security-Abdeckung auf 790 Source-/Test-/Manifest-Einfügungen reduziert. Alle Repository-Gates einschließlich echtem PostgreSQL 16.9, SQLite, Supply Chain, Frontend, Playwright und Release-Build sind grün; der vollständige Rust-Workspace-Test wurde wegen einer dokumentierten Windows-Smart-App-Control-Blockade zusätzlich erfolgreich in einem kurzlebigen Rust-1.95.0-Linux-Container ausgeführt. Der v0.1-Readiness-Runner bleibt wegen 15 geplanter Szenarien ehrlich rot. Patch/Revoke-HTTP folgt in `IAM-041`, die Produktionskomposition in `IAM-013`; unabhängige commit-gebundene Review-/CI-Evidence fehlt. Details stehen in `docs/implementation-evidence/iam-040-api-token-create-http.md`.
