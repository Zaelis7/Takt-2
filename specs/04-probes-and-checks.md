...
|---|---|---|---|
| HTTP/TCP/DNS/ICMP/TLS/Browser | `resolver`, `address_family` | `resolver`, `addressFamily` | optionaler Resolver als `udp`-/`tcp`-/`tls`-URI, maximal 2048 Zeichen; Familie `auto`, `ipv4` oder `ipv6`, Standard `auto` |
| HTTP/TCP/TLS/Browser | `proxy.url`, `proxy.auth` | `proxy.url`, `proxy.auth` | authority-only URI mit `http`, `https` oder `socks5`, maximal 2048 Zeichen; Zugangsdaten nicht in der URL |
| Proxy Auth | `username`, `password` | `username`, `password` | beide Werte sind verpflichtende SecretRefs; Proto enthält ausschließlich kurzlebige `ephemeral_key`-Referenzen |
...
Resolver-URIs enthalten nur Authority und optional einen abschließenden Slash; Userinfo, Pfad, Query und Fragment sind unzulässig. Ohne Resolver gilt die sichere Executor-Voreinstellung. `address_family` steuert die Verbindung zum Ziel, Resolver oder Proxy, ändert bei DNS-Checks aber nicht den angefragten Record-Typ. Proxy-URLs folgen derselben Authority-only-Regel und dürfen keine Zugangsdaten enthalten. Proxy-Unterstützung ist auf HTTP, TCP, TLS und Browser begrenzt; DNS und ICMP verwenden nur Resolver und Adressfamilie, Push besitzt keine ausgehende Netzwerkoption.

Config-Secret-Referenzen bestehen aus `secretRef` und optionalem `key` (Standard `value`), OpenAPI aus `secret_ref` und `key`. Vor Probe-Dispatch MUSS der Server sie zuletzt verantwortlich auflösen und nur den kurzlebigen `ephemeral_key` im versiegelten Secret-Bundle und in Proto-Nachrichten verwenden. Literal-Secrets in HTTP- oder Proxy-`auth` sowie Browser-`fill` sind ungültig. Proto-Felder mit einem von null verschiedenen Standard und zugleich sinnvoller expliziter Null-/False-Semantik verwenden `optional`; andere fehlende numerische Werte und leere optionale String-Defaults werden vor Eintritt in die Domäne auf die Tabelle normalisiert. Leere Pflichtstrings bleiben ungültig. Ungültige Proto-Werte ergeben `ACK_DISPOSITION_REJECTED_INVALID`, niemals eine Observation mit `TARGET_FAILURE`.
